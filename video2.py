...
def vedo():
    ...
    label_available_doctors = tk.Label(root, text="उपलब्ध डॉक्टर", font=("Arial", 18, "bold"), bg="#e6e6e6")
    ...
    def meet_doctor(doctor_num):
        ...
        if start_time <= current_time <= end_time:
            ...
            messagebox.showerror("डॉक्टर उपलब्ध नाहीत", f"Doctor {doctors_data[doctor_num]['name']} उपलब्ध नाही.")
            send_email(doctor_name)
    ...
    for i, doctor in enumerate(doctors_data):
        ...
        button = tk.Button(frame, text=f"भेटा {doctor_names[i]}", command=lambda num=i: meet_doctor(num))
        button.pack()